...
RSpec.describe Entries::Filters::Sqlite3Filter, type: :service do
  ...
  let(:matches_filter) { create(:filter, user: user, comparison: Filter::MATCHES_FILTER, value: "f.*z") }
  ...
  context "when there's more than one filter" do
    let(:second_matches_filter) { create(:filter, user: user, comparison: Filter::MATCHES_FILTER, value: "q.*x") }
    
    before do
      second_matches_filter
    end
