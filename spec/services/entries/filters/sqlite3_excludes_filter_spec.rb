...
RSpec.describe Entries::Filters::Sqlite3Filter, type: :service do
  ...
  before do
    ...
    create(:entry, feed: feed, title: "foo polaf")
  ...
  it "filters" do
    ...
    expect(filter_entries.scope.pluck(:title)).to include "foo polaf"
    expect(filter_entries.scope.pluck(:title)).not_to include "quux qoox"
  end

  context "when there's more than one filter" do
    let(:second_excludes_filter) { create(:filter, user: user, comparison: Filter::EXCLUDES_FILTER, value: "ba") }
    
    before do
      second_excludes_filter
    end
